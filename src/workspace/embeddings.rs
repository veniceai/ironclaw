...
/// Venice AI embedding provider using the Venice OpenAI-compatible API.
///
/// Uses static API key auth, same as the Venice LLM provider.
pub struct VeniceEmbeddings {
    client: reqwest::Client,
    api_key: String,
    base_url: String,
    model: String,
...
impl VeniceEmbeddings {
    /// Create a new Venice embedding provider.
    ///
    /// Uses text-embedding-3-small which has 1536 dimensions.
    /// The `base_url` should already include `/api/v1` (e.g., `https://api.venice.ai/api/v1`).
    ...
    /// Use a specific model with the given dimension.
    pub fn with_model(mut self, model: impl Into<String>, dimension: usize) -> Self {
        self.model = model.into();
        self.dimension = dimension;
        self
...
#[derive(Debug, Serialize)]
struct VeniceEmbeddingRequest<'a> {
    model: &'a str,
    input: &'a [String],
}

#[derive(Debug, Deserialize)]
struct VeniceEmbeddingResponse {
    data: Vec<VeniceEmbeddingData>,
}

#[derive(Debug, Deserialize)]
struct VeniceEmbeddingData {
    embedding: Vec<f32>,
}

#[async_trait]
impl EmbeddingProvider for VeniceEmbeddings {
    fn dimension(&self) -> usize {
        self.dimension
    }
    ...
    async fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, EmbeddingError> {
        ...
        let request = VeniceEmbeddingRequest {
            model: &self.model,
            input: texts,
        };
        ...
        let result: VeniceEmbeddingResponse = response.json().await.map_err(|e| {
            EmbeddingError::InvalidResponse(format!("Failed to parse response: {}", e))
        })?;
...
mod tests {
    ...
    #[test]
    fn test_venice_embeddings_config() {
        let provider =
            VeniceEmbeddings::new("test-key", "https://api.venice.ai/api/v1");
        assert_eq!(provider.dimension(), 1536);
        assert_eq!(provider.model_name(), "text-embedding-3-small");
